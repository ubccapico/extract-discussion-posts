...
def get_full_topic_group_discussion(group_id,topic_id):
    full_topic = requests.get(
        init.base_url + '/api/v1/groups/{}/discussion_topics/{}/view'.format(
            str(group_id), str(topic_id)),
        params={'access_token': init.access_token})
    return full_topic.json()

def get_group_discussion(group_id,topic_id):
    group_discussion = requests.get(
        init.base_url + '/api/v1/groups/{}/discussion_topics/{}'.format(
            str(group_id), str(topic_id)),
        params={'access_token': init.access_token})
    return group_discussion.json()
